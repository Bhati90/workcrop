...
from django.conf import settings
from .utils.fcm_helper import send_push_notification
from .serializers import (SaveFCMTokenSerializer,CropSerializer, CropVarietySerializer, CropWithVarietiesSerializer)
...
from .serializers import CropSerializer, CropVarietySerializer
...
class CropVarietyViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        """Filter varieties by crop if crop parameter is provided"""
        queryset = CropVariety.objects.all()
...
class ActivityViewSet(viewsets.ModelViewSet):
    queryset = Activity.objects.select_related('crop').all()  # ✅ ADD select_related
    serializer_class = ActivitySerializer
    filter_backends = [DjangoFilterBackend, filters.SearchFilter, filters.OrderingFilter]
    filterset_fields = ['crop']  # ✅ ADD
    search_fields = ['name', 'description', 'crop__name']  # ✅ ADD crop__name
    ordering_fields = ['name', 'days_after_pruning', 'created_at']
    ordering = ['name']
    
    @action(detail=False, methods=['get'])
    def by_crop(self, request):
        """
        Get activities filtered by crop
        GET /api/activities/by_crop/?crop_id={uuid}
        """
        crop_id = request.query_params.get('crop_id')
        
        if crop_id:
            activities = Activity.objects.filter(crop_id=crop_id)
        else:
...
class MukadamActivityRateViewSet(viewsets.ModelViewSet):
    ...
    # views.py - Fix bulk_create in MukadamActivityRateViewSet

    @action(detail=False, methods=['post'])
    def bulk_create(self, request):
        """
        Bulk create/update activity rates for a mukadam
        POST /api/mukadam-activity-rates/bulk_create/
        Body: {
            "mukadam_id": "uuid",
            "rates": [
                {
                    "activity_id": "uuid",  // ✅ Changed from activity to activity_id
                    "rate_per_acre": 1200,
                    "is_available": true
                }
            ]
        ...
        if not mukadam_id:
            return Response({
                "error": "mukadam_id is required"
            }, status=400)
        
        # ✅ Validate mukadam exists
        try:
            mukadam = Mukadam.objects.get(id=mukadam_id)
        except Mukadam.DoesNotExist:
            return Response({
                "error": f"Mukadam with ID {mukadam_id} not found"
            }, status=404)
        
        if not rates_data:
            return Response({
                "error": "rates array is required and cannot be empty"
            }, status=400)
        ...
        for rate_data in rates_data:
            # ✅ Get activity_id and validate
            activity_id = rate_data.get('activity_id') or rate_data.get('activity')
            
            if not activity_id or activity_id == 'undefined' or activity_id == 'null':
                errors.append({
                    'error': 'Invalid or missing activity_id',
                    'data': rate_data
                })
                continue
            
            rate_per_acre = rate_data.get('rate_per_acre')
            is_available = rate_data.get('is_available', True)
            
            # ✅ Validate rate
            if not rate_per_acre or rate_per_acre <= 0:
                errors.append({
                    'activity_id': activity_id,
                    'error': 'rate_per_acre must be a positive number',
                    'data': rate_data
                })
                continue
...
class FarmerViewSet(viewsets.ModelViewSet):
    ...
    def _sync_plots_from_jobs(self):
        """Create missing plots from jobs for all farmers - WITH CROP INFO"""
        jobs_without_plots = Job.objects.select_related(
            'farmer', 'activity', 'crop', 'crop_variety'  # ✅ ADD
        ).all()
        ...
        for job in jobs:
            ...
            if existing_plot:
                ...
                FarmerPlot.objects.create(
                    farmer=job.farmer,
                    crop=job.crop,  # ✅ ADD
                    crop_variety=job.crop_variety,  # ✅ ADD
                    activity=job.activity,  # ✅ ADD (instead of activity_name)
                    defaults={
                        'acres': job.farm_size_acres,
                        'location': job.location or job.farmer.village,
                        'pruning_date': job.requested_date,
                        'activity_name': job.activity.name,  # Keep for legacy
                        'notes': f"Auto-synced from jobs"
                    }
                )
                ...
                if not created and job.farm_size_acres > plot.acres:
                    ...
                    plot.save()
    def _sync_farmer_plots(self, farmer):
        """Create missing plots for specific farmer from their jobs - WITH CROP INFO"""
        jobs = Job.objects.filter(farmer=farmer).select_related(
            'activity', 'crop', 'crop_variety'  # ✅ ADD
        )
...
class JobViewSet(viewsets.ModelViewSet):
    ...
    def simple_list(self, request):
        ...
        jobs = Job.objects.select_related(
            'farmer', 'activity', 'crop', 'crop_variety',  # ✅ ADD crop fields
            'assigned_mukadam'
        ).prefetch_related(
            'interests__mukadam'
        ).all()
        ...
        for job in jobs:
            ...
            job_info = {
                ...
                'crop': job.crop.name if job.crop else None,  # ✅ ADD
                'crop_variety': job.crop_variety.name if job.crop_variety else None,  # ✅ ADD
                'farm_size_acres': float(job.farm_size_acres),
                'location': job.location,
                'requested_date': str(job.requested_date),
                'farmer_price_per_acre': float(job.farmer_price_per_acre),
...
# views.py - Add to JobViewSet

    @action(detail=False, methods=['post'])
    def bulk_assign(self, request):
        """
        Assign multiple jobs to a mukadam at once
        POST /api/jobs/bulk_assign/
        Body: {
            "job_ids": ["uuid1", "uuid2", "uuid3"],
            "mukadam_id": "mukadam-uuid"
        }
        """
        job_ids = request.data.get('job_ids', [])
        mukadam_id = request.data.get('mukadam_id')
        
        if not job_ids or not mukadam_id:
            return Response({
                "error": "job_ids (array) and mukadam_id are required"
            }, status=400)
        ...
        # Check if all jobs are in notified status and have interest from this mukadam
        assigned_jobs = []
        failed_jobs = []
        
        for job in jobs:
            # Check if mukadam showed interest
            interest = MukadamInterest.objects.filter(
                job=job,
                mukadam=mukadam,
                is_interested=True
            ).first()
            ...
            if not interest:
                failed_jobs.append({
                    'job_id': str(job.id),
                    'reason': 'Mukadam not interested or not notified'
                })
                continue
            ...
            if job.status != 'notified':
                failed_jobs.append({
                    'job_id': str(job.id),
                    'reason': f'Job status is {job.status}, must be notified'
                })
                continue
        ...
        return Response({
            'message': f'Successfully assigned {len(assigned_jobs)} jobs to {mukadam.name}',
            'mukadam': {
                'id': str(mukadam.id),
                'name': mukadam.name
            },
            ...
            'summary': {
                ...
                'successfully_assigned': len(assigned_jobs),
                'failed': len(failed_jobs)
            }
        })


    # views.py - Update auto_price method

    @action(detail=True, methods=['post'])
    def auto_price(self, request, pk=None):
        """Auto-set price based on company activity rates"""
        job = self.get_object()
        
        if job.status != 'confirmed':
            return Response({
                "error": f"Job must be in 'confirmed' status. Current status: {job.status}"
            }, status=400)
        ...
        try:
            ...
            your_rate = float(company_rate.rate_per_acre)
        except CompanyActivityRate.DoesNotExist:
            return Response({
                "error": f"No company rate configured for activity: {job.activity.name}",
                "suggestion": "Please configure company rates in settings or set price manually"
            }, status=400)
        
        # Allow manual override
        if 'your_rate_per_acre' in request.data:
            your_rate = float(request.data['your_rate_per_acre'])
        
        # Validate rate
        if your_rate > job.farmer_price_per_acre:
            return Response({
                "error": "Your rate cannot exceed farmer's budget",
                "your_rate": your_rate,
                "farmer_rate": float(job.farmer_price_per_acre),
                "suggestion": f"Please set rate at or below ₹{job.farmer_price_per_acre}/acre"
            }, status=400)
        
        # Set the price
        job.your_price_per_acre = your_rate
        job.status = 'priced'
        job.save()
        ...
        JobStatusHistory.objects.create(
            ...
            to_status='priced',
            changed_by=request.user if request.user.is_authenticated else None,
            notes=f'Auto-priced at ₹{your_rate}/acre using company rate'
        )
        
        # Calculate profit
        margin = (job.farmer_price_per_acre - your_rate) * job.farm_size_acres
        margin_percent = ((job.farmer_price_per_acre - your_rate) / job.farmer_price_per_acre) * 100
        
        return Response({
            'success': True,
            'message': 'Price set automatically',
            'pricing': {
                'your_price_per_acre': float(your_rate),
                'farmer_price_per_acre': float(job.farmer_price_per_acre),
                'farm_size_acres': float(job.farm_size_acres),
                'total_payment_to_mukadam': float(your_rate * job.farm_size_acres),
                'total_from_farmer': float(job.farmer_price_per_acre * job.farm_size_acres),
                'your_margin': float(margin),
    ...
    @action(detail=True, methods=['get'])
    def recommended_mukadams(self, request, pk=None):
        """
        Get recommended mukadams based on:
        1. Activity rate match
        2. Team size match
        3. Availability on job date
        4. Past performance
        
        GET /api/jobs/{id}/recommended_mukadams/
        """
        job = self.get_object()
        ...
        all_mukadams = Mukadam.objects.filter(is_active=True).prefetch_related(
        ...
        for mukadam in all_mukadams:
            score = 0
            reasons = []
            flags = []
            
            # 1. Check if they have rate for this activity
            ...
            if not activity_rate:
                continue  # Skip if they don't do this activity
            
            mukadam_rate = float(activity_rate.rate_per_acre)
            
            # 2. Rate compatibility (within your budget)
            if mukadam_rate <= job.your_price_per_acre:
                score += 30
                reasons.append(f"Rate fits budget (₹{mukadam_rate}/acre)")
                
                # Bonus for being significantly under budget
                if mukadam_rate <= job.your_price_per_acre * 0.9:
                    score += 10
                    reasons.append("Great value - 10%+ under budget")
            else:
                flags.append(f"Rate too high (₹{mukadam_rate}/acre vs budget ₹{job.your_price_per_acre}/acre)")
                continue  # Skip if over budget
            
            # 3. Team size compatibility
            if mukadam.number_of_labourers >= (job.workers_needed or 5):
                score += 20
                reasons.append(f"Team size adequate ({mukadam.number_of_labourers} workers)")
            else:
                score += 5
                flags.append(f"Small team ({mukadam.number_of_labourers} workers, need {job.workers_needed})")
            
            # 4. Availability on job date
            has_availability = MukadamAvailability.objects.filter(
                mukadam=mukadam,
                start_date__lte=job.requested_date,
                end_date__gte=job.requested_date
            ).exists()
            
            has_job = Job.objects.filter(
                finalized_mukadam=mukadam,
                requested_date=job.requested_date,
                status__in=['finalized', 'assigned', 'in_progress']
            ).exists()
            
            if has_availability and not has_job:
                score += 25
                reasons.append("Available on job date")
            elif not has_availability:
                score -= 15
                flags.append("No availability period set for this date")
            elif has_job:
                score -= 30
                flags.append("Already has job on this date")
            ...
            if past_jobs > 10:
                score += 15
                reasons.append(f"Experienced ({past_jobs} jobs completed)")
            elif past_jobs > 5:
                score += 10
                reasons.append(f"Reliable ({past_jobs} jobs completed)")
            elif past_jobs > 0:
                score += 5
                reasons.append(f"Some experience ({past_jobs} jobs)")
            
            # 6. Location proximity (if you want to add this)
            # You could calculate distance between job.location and mukadam.location
            ...
            recommendations.append({
                'mukadam': {
                    ...
                    'location': mukadam.location,
                    'team_size': mukadam.number_of_labourers,
                    'rate_for_activity': mukadam_rate
                },
                'score': score,
                'reasons': reasons,
                'flags': flags,
                'is_available': has_availability and not has_job,
                'cost_estimate': {
                    'rate_per_acre': mukadam_rate,
                    'total_cost': mukadam_rate * float(job.farm_size_acres),
                    'your_margin': (float(job.your_price_per_acre) - mukadam_rate) * float(job.farm_size_acres)
                }
            })
        ...
        return Response({
            'job': {
                ...
                'farm_size': float(job.farm_size_acres),
                'requested_date': str(job.requested_date),
                'your_budget': float(job.your_price_per_acre)
            },
            'recommendations': recommendations,
            'summary': {
                'total_mukadams': len(recommendations),
                'highly_recommended': len([r for r in recommendations if r['score'] >= 60]),
                'available': len([r for r in recommendations if r['is_available']]),
                'within_budget': len(recommendations)
            }
        })
...
class MukadamViewSet(viewsets.ModelViewSet):
    ...
    def job_history(self, request, pk=None):
        ...
        interest_records = MukadamInterest.objects.filter(
            ...
            'job', 'job__farmer', 'job__activity',
            'job__crop', 'job__crop_variety'  # ✅ ADD
        ).order_by('-job__requested_date')
    ...
    # views.py - Add to MukadamViewSet

   # views.py - REPLACE the list_with_availability method in MukadamViewSet

    @action(detail=False, methods=['get'])
    def list_with_availability(self, request):
        """
        Get all mukadams with their availability periods
        GET /api/mukadams/list_with_availability/
        """
        mukadams = Mukadam.objects.filter(is_active=True).prefetch_related(
            'activity_rates__activity',
            'availability_periods'  # ✅ ADD
        ).order_by('name')
        ...
        for mukadam in mukadams:
            # Get activity rates
            activity_rates = []
            for rate in mukadam.activity_rates.filter(is_available=True):
            ...
            availability_periods = []
            for period in mukadam.availability_periods.all():
            ...
            # ✅ Check if currently available (has availability period covering today AND no job)
            has_availability_today = MukadamAvailability.objects.filter(
                mukadam=mukadam,
                start_date__lte=today,
                end_date__gte=today
            ).exists()
            
            has_job_today = Job.objects.filter(
                finalized_mukadam=mukadam,
                requested_date=today,
                status__in=['finalized', 'assigned', 'in_progress']
            ).exists()
            
            is_currently_available = has_availability_today and not has_job_today
            
            mukadam_data = {
                'id': str(mukadam.id),
                'mukkadam_name': mukadam.name,
                'contact': mukadam.phone,
                'location': mukadam.location,
                'number_of_labourers': mukadam.number_of_labourers,
                'is_active': mukadam.is_active,
                'activity_rates': activity_rates,
                'availability_periods': availability_periods,  # ✅ ADD
                'is_currently_available': is_currently_available
            }
            
            mukadam_list.append(mukadam_data)
        
        return Response(mukadam_list)


    # views.py - REPLACE available_for_date_range in MukadamViewSet

    @action(detail=False, methods=['get'])
    def available_for_date_range(self, request):
        """
        Get mukadams available for a date range (must have availability period AND no job)
        GET /api/mukadams/available_for_date_range/?start_date=2025-11-19&end_date=2025-11-25
        """
        from datetime import datetime, timedelta
        ...
        if not start_date_str:
            return Response({
                "error": "start_date parameter required (YYYY-MM-DD format)"
            }, status=400)
        ...
        available_mukadams = []
        unavailable_mukadams = []
        
        for mukadam in mukadams:
            # ✅ Check if mukadam has availability covering the entire date range
            has_availability = MukadamAvailability.objects.filter(
                mukadam=mukadam,
            ...
            if not has_availability:
                # Not available - no availability period covering this range
                unavailable_mukadams.append({
                    'id': str(mukadam.id),
                    'mukkadam_name': mukadam.name,
                    'contact': mukadam.phone,
                    'location': mukadam.location,
                    'number_of_labourers': mukadam.number_of_labourers,
                    'reason': 'No availability period set for these dates'
                })
                continue
            
            # Check if mukadam has any job in the date range
            has_job = Job.objects.filter(
                finalized_mukadam=mukadam,
            ...
            # Get availability periods
            availability_periods = []
            for period in mukadam.availability_periods.filter(
            ...
            if has_job:
                ...
                jobs = Job.objects.filter(
                    ...
                    requested_date__lte=end_date,
                    status__in=['finalized', 'assigned', 'in_progress']
                ).select_related('farmer', 'activity')
                
                mukadam_data['busy_dates'] = [
                    {
                ...
                unavailable_mukadams.append(mukadam_data)
            else:
                available_mukadams.append(mukadam_data)
        ...
        return Response({
            ...
            'available_count': len(available_mukadams),
            'unavailable_count': len(unavailable_mukadams),
            'total_count': len(mukadams),
            'available_mukadams': available_mukadams,
            'unavailable_mukadams': unavailable_mukadams
        })
    @action(detail=True, methods=['get'])
    def detailed_stats(self, request, pk=None):
        """
    ...
    # views.py - REPLACE the calendar_availability method in MukadamViewSet

    @action(detail=False, methods=['get'])
    def calendar_availability(self, request):
        """
        Get team availability calendar view based on availability periods
        GET /api/mukadams/calendar_availability/?start_date=2025-11-01&end_date=2025-11-30
        """
        from datetime import datetime, timedelta
        ...
        while current_date <= end_date:
            ...
            calendar_data[date_str]['day_name'] = current_date.strftime('%A')
            calendar_data[date_str]['day_number'] = current_date.day
            
            # Check each mukadam's availability for this date
            for mukadam in mukadams:
                # ✅ Check if mukadam has availability period covering this date
                has_availability = MukadamAvailability.objects.filter(
                    mukadam=mukadam,
                ...
                if not has_availability:
                    continue  # Skip if not in any availability period
                
                # ✅ Check if mukadam has any job on this date
                has_job = Job.objects.filter(
                    finalized_mukadam=mukadam,
                ...
                if not has_job:
                    ...
                    # Add activity rates
                    for rate in mukadam.activity_rates.filter(is_available=True):
                        team_info['activity_rates'].append({
                            'activity': rate.activity.name,
                            'rate_per_acre': float(rate.rate_per_acre)
                        })
                    
                    calendar_data[date_str]['teams'].append(team_info)
                    calendar_data[date_str]['available_count'] += 1
        ...
        summary = {
            ...
            'total_teams': mukadams.count(),
            'avg_available_per_day': sum(day['available_count'] for day in calendar_list) / len(calendar_list) if calendar_list else 0,
            'peak_availability': max((day['available_count'] for day in calendar_list), default=0),
            'low_availability_days': len([day for day in calendar_list if day['available_count'] < 3])
        }
    ...
    # views.py - Add to MukadamViewSet
# views.py - Replace/Add to MukadamViewSet

    @action(detail=True, methods=['get'])
    def availability_periods(self, request, pk=None):
        """
        Get all availability periods for a mukadam
        GET /api/mukadams/{id}/availability_periods/
        """
        mukadam = self.get_object()
        
        # Get all availability periods
        availability_periods = MukadamAvailability.objects.filter(
            mukadam=mukadam
        ).order_by('start_date')
        
        periods_data = []
        for period in availability_periods:
            # Check if they have any job during this period
            has_job = Job.objects.filter(
                finalized_mukadam=mukadam,
                requested_date__gte=period.start_date,
                requested_date__lte=period.end_date,
                status__in=['finalized', 'assigned', 'in_progress']
            ).exists()
            ...
            periods_data.append({
                ...
                'end_date': str(period.end_date),
                'notes': period.notes,
                'has_job_in_period': has_job,
                'created_at': period.created_at.isoformat()
            })
        
        # Check if currently available
        today = timezone.now().date()
        is_currently_available = MukadamAvailability.objects.filter(
            mukadam=mukadam,
            start_date__lte=today,
            end_date__gte=today
        ).exists()
        ...
        upcoming_jobs = Job.objects.filter(
            ...
            status__in=['finalized', 'assigned', 'in_progress']
        ).select_related('farmer', 'activity').order_by('requested_date')
        ...
        return Response({
            ...
            'is_currently_available': is_currently_available,
            'availability_periods': periods_data,
            'upcoming_jobs': jobs_data
        })
    ...
    def add_availability(self, request, pk=None):
        """
        Add availability period for a mukadam
        POST /api/mukadams/{id}/add_availability/
        Body: {
            "start_date": "2025-11-20",
            "end_date": "2025-11-30",
            "notes": "Available for all jobs"
        }
        """
        ...
        overlapping = MukadamAvailability.objects.filter(
            mukadam=mukadam,
            start_date__lte=end_date,
            end_date__gte=start_date
        )
        ...
        if overlapping.exists():
            return Response({
                "error": "This period overlaps with an existing availability period",
                "overlapping_periods": [
                    {
        ...
        availability = MukadamAvailability.objects.create(
            mukadam=mukadam,
    ...
    @action(detail=True, methods=['delete'], url_path='availability_periods/(?P<period_id>[^/.]+)')
    def delete_availability(self, request, pk=None, period_id=None):
        """
        Delete an availability period
        DELETE /api/mukadams/{id}/availability_periods/{period_id}/
        """
        mukadam = self.get_object()
        
        try:
            period = MukadamAvailability.objects.get(
                id=period_id,
                mukadam=mukadam
            )
            period.delete()
        ...
        except MukadamAvailability.DoesNotExist:
            return Response({
    ...
    def mark_unavailable(self, request, pk=None):
        """
        Mark mukadam as unavailable for specific dates
        POST /api/mukadams/{id}/mark_unavailable/
        Body: {
            "start_date": "2025-11-20",
            "end_date": "2025-11-25",
        ...
        # Create a special "blocked" job entry or use a separate model
        # For simplicity, we'll track this in notes
        # You might want to create a MukadamUnavailability model instead
        ...
        while current_date <= end_date:
            ...
            existing = Job.objects.filter(
                finalized_mukadam=mukadam,
                requested_date=current_date,
                notes__contains='[BLOCKED]'
            ).exists()
            
            if not existing:
                blocked_dates.append(str(current_date))
            
            current_date += timedelta(days=1)
        
        return Response({
            'message': f'Marked unavailable for {len(blocked_dates)} dates',
            'mukadam': mukadam.name,
    ...
    def list(self, request, *args, **kwargs):
        """Enhanced list with recommendation scoring"""
        detailed = request.query_params.get('detailed', 'false').lower() == 'true'
        job_id = request.query_params.get('job_id')
        
        mukadams = self.get_queryset().filter(is_active=True)
        result_data = []
        ...
        for mukadam in mukadams:
            ...
            if detailed or job_id:
                # Get activity rates
                activity_rates = MukadamActivityRate.objects.filter(
                    mukadam=mukadam,
                    is_available=True
                )
                ...
                # ✅ Get stats using MukadamInterest
                all_interests = MukadamInterest.objects.filter(mukadam=mukadam)
                data['total_jobs'] = all_interests.count()
                
                # ✅ Using assigned_mukadam
                data['completed_jobs'] = Job.objects.filter(
                    assigned_mukadam=mukadam, 
                    status='completed'
                ).count()
                
                data['won_bids'] = all_interests.filter(response_status='assigned').count()
                
                # ✅ Using assigned_mukadam for finalized jobs
                avg_price = Job.objects.filter(
                    assigned_mukadam=mukadam,
                    your_price_per_acre__isnull=False
                ).aggregate(avg=Avg('your_price_per_acre'))['avg']
                data['avg_bid_price'] = round(float(avg_price), 2) if avg_price else 0
            
            # ADD RECOMMENDATION SCORING
            if job_id:
                try:
                    job = Job.objects.get(id=job_id)
                    score = self._calculate_recommendation_score(mukadam, job)
        ...
        # Sort by recommendation score if job_id provided
        if job_id:
            result_data.sort(key=lambda x: x.get('recommendation', {}).get('score', 0), reverse=True)
        
        return Response(result_data)
        
    # views.py - Update the _calculate_recommendation_score method
    def _calculate_recommendation_score(self, mukadam, job):
        """Calculate recommendation score for mukadam-job pairing"""
        score = 0
        reasons = []
        flags = []
        is_recommended = False
        
        # 1. Check if mukadam does this activity
        try:
            activity_rate = MukadamActivityRate.objects.get(
                mukadam=mukadam,
                activity=job.activity,
                is_available=True
            )
            mukadam_rate = activity_rate.rate_per_acre  # ✅ Keep as Decimal
        except MukadamActivityRate.DoesNotExist:
            return {
                'score': 0,
                'reasons': [],
                'flags': ['Does not do this activity'],
                'is_available': False,
                'is_recommended': False,
                'rate': None
            }
        
        # 2. PRICE COMPATIBILITY - Most Important (40 points)
        if job.your_price_per_acre:
            your_price = job.your_price_per_acre  # ✅ Already Decimal
            
            if mukadam_rate <= your_price:
                # ✅ Convert to float for percentage calculation
                price_diff_percent = float((your_price - mukadam_rate) / your_price * 100)
                
                if price_diff_percent >= 20:
                    score += 40
                    reasons.append(f"Excellent value - {int(price_diff_percent)}% under budget")
                elif price_diff_percent >= 10:
                    score += 35
                    reasons.append(f"Good value - {int(price_diff_percent)}% under budget")
                elif price_diff_percent >= 5:
                    score += 30
                    reasons.append(f"Fair price - {int(price_diff_percent)}% under budget")
                else:
                    score += 25
                    reasons.append("Within budget")
            else:
                # ✅ Convert to float for percentage calculation
                over_percent = float((mukadam_rate - your_price) / your_price * 100)
                flags.append(f"Rate {int(over_percent)}% over budget (₹{mukadam_rate} vs ₹{your_price})")
                return {
                    'score': 0,
                    'reasons': reasons,
                    'flags': flags,
                    'is_available': False,
                    'is_recommended': False,
                    'rate': float(mukadam_rate),
                    'rate_comparison': 'over_budget'
                }
        
        # 3. AVAILABILITY ON JOB DATE (30 points)
        has_availability = MukadamAvailability.objects.filter(
            mukadam=mukadam,
            start_date__lte=job.requested_date,
            end_date__gte=job.requested_date
        ...
        has_conflict = Job.objects.filter(
            assigned_mukadam=mukadam,
            requested_date=job.requested_date,
            status__in=['assigned', 'in_progress', 'completed']
        ).exclude(id=job.id).exists()
        
        if has_availability and not has_conflict:
            score += 30
            reasons.append("✅ Available on job date")
            is_available = True
        elif not has_availability and not has_conflict:
            score += 10
            flags.append("⚠️ No availability period set (may still be available)")
            is_available = True
        elif has_conflict:
            score -= 40
            flags.append("❌ Already booked on this date")
            is_available = False
        else:
            flags.append("❌ Not available on this date")
            is_available = False
        
        # 4. TEAM SIZE MATCH (15 points)
        if job.workers_needed:
            if mukadam.number_of_labourers >= job.workers_needed:
                size_diff = mukadam.number_of_labourers - job.workers_needed
                if size_diff <= 2:
                    score += 15
                    reasons.append(f"Perfect team size ({mukadam.number_of_labourers} workers)")
                elif size_diff <= 5:
                    score += 12
                    reasons.append(f"Good team size ({mukadam.number_of_labourers} workers)")
                else:
                    score += 8
                    reasons.append(f"Large team ({mukadam.number_of_labourers} workers)")
            else:
                shortage = job.workers_needed - mukadam.number_of_labourers
                score += 3
                flags.append(f"Team smaller than needed (need {job.workers_needed}, has {mukadam.number_of_labourers})")
        ...
        total_assigned = MukadamInterest.objects.filter(
            mukadam=mukadam,
        ...
        if completed_jobs >= 20:
            score += 15
            reasons.append(f"🌟 Highly experienced ({completed_jobs} jobs)")
        elif completed_jobs >= 10:
            score += 12
            reasons.append(f"Experienced ({completed_jobs} jobs)")
        elif completed_jobs >= 5:
            score += 8
            reasons.append(f"Reliable ({completed_jobs} jobs)")
        elif completed_jobs > 0:
            score += 5
            reasons.append(f"Some experience ({completed_jobs} jobs)")
        else:
            score += 2
            flags.append("New mukadam (no completed jobs)")
        
        # 6. RESPONSE HISTORY (10 points)
        all_interests = MukadamInterest.objects.filter(mukadam=mukadam)
        total_notified = all_interests.count()
        
        if total_notified > 0:
            responded = all_interests.exclude(response_status='pending').count()
            response_rate = (responded / total_notified) * 100
            
            if response_rate >= 90:
                score += 10
                reasons.append(f"Excellent response rate ({int(response_rate)}%)")
            elif response_rate >= 70:
                score += 7
                reasons.append(f"Good response rate ({int(response_rate)}%)")
            elif response_rate >= 50:
                score += 4
                reasons.append(f"Fair response rate ({int(response_rate)}%)")
            else:
                flags.append(f"Low response rate ({int(response_rate)}%)")
        
        # 7. LOCATION PROXIMITY (5 points)
        if mukadam.location.lower() == job.location.lower():
            score += 5
            reasons.append("📍 Same location as job")
        
        # Determine recommendation level
        if score >= 70 and is_available:
            is_recommended = True
            recommendation_level = "highly_recommended"
        elif score >= 50 and is_available:
            is_recommended = True
            recommendation_level = "recommended"
        elif score >= 30:
            recommendation_level = "suitable"
        else:
            recommendation_level = "not_suitable"
        
        # ✅ Calculate cost estimate with proper Decimal handling
        margin = Decimal('0')
        if job.your_price_per_acre:
            margin = (job.your_price_per_acre - mukadam_rate) * job.farm_size_acres
        ...
        return {
            ...
            'rate': float(mukadam_rate),  # ✅ Convert to float for JSON
            'rate_comparison': 'within_budget',
            'cost_estimate': {
                'rate_per_acre': float(mukadam_rate),
                'total_cost': float(mukadam_rate * job.farm_size_acres),
                'your_margin': float(margin)
            }
        }
            ...
            for interest in job.interests.all():
                response_details.append({
                    ...
                    'response': {
                        'is_interested': interest.is_interested,
                        'responded_at': interest.responded_at.isoformat() if interest.responded_at else None,
                        'status': interest.response_status
                    }
                })
...
class MukadamProfileViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def _get_activity_breakdown(self, mukadam):
        """Get breakdown by activity type and crop"""
        from django.db.models import Count
        ...
        breakdown = []
        for stat in activity_stats:
            activity_name = stat['activity__name']
            crop_name = stat.get('crop__name', 'Not specified')
            
            # Calculate earnings for this activity + crop combo
            activity_jobs = Job.objects.filter(
                assigned_mukadam=mukadam,
            ...
            if crop_name != 'Not specified':
                activity_jobs = activity_jobs.filter(crop__name=crop_name)
